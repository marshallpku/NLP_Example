...
heq <- function(x, ...) {
  ...
  getpv <- function(x, flowtype, plist) {
    ...
    pv.cons <- plist$pvf(yvals, plist$dr)
    return(pv.cons)
  }
...
hin <- function(x, ...) {
  ...
  yvals.actives <- plist$gauss(x[plist$iactives], plist$years)
  actives.m1 <- plist$pvf(yvals.actives, plist$dr - .01)
  actives.p1 <- plist$pvf(yvals.actives, plist$dr + .01)
  
  yvals.retired <- plist$gauss(x[plist$iretired], plist$years)
  retired.m1 <- plist$pvf(yvals.retired, plist$dr - .01)
  retired.p1 <- plist$pvf(yvals.retired, plist$dr + .01)
  
  yvals.termv <- plist$gauss(x[plist$itermv], plist$years)
  termv.m1 <- plist$pvf(yvals.termv, plist$dr - .01)
  termv.p1 <- plist$pvf(yvals.termv, plist$dr + .01)
...
#****************************************************************************************************
#                ...Alternatively, read plan data from file ####
#****************************************************************************************************
fname <- "plans(3).xlsx"
df <- read_excel(paste0("./data-raw/", fname), sheet="plans", skip=1)
df
glimpse(df)
...
the_plan <- plans.df %>% filter(planid==theplanid) %>% select(-pv_total_data, -year1_total_data) %>%
  ...
  mutate_each(funs(./scale), -planid, -planname, -dr, -actives_age, -age_at_retire, -scale)
glimpse(the_plan)
...
plist$duration <- function(vec, dr) {
  pv <- plist$pvf(vec, dr)
  pv.m1 <- plist$pvf(vec, dr - .01)
  pv.p1 <- plist$pvf(vec, dr + .01)
  duration <- (pv.m1 - pv.p1) / (2*pv) * 100
  return(duration)
}
...
plist$pv_payroll <- the_plan$pv_payroll
plist$year1_payroll <- the_plan$year1_payroll
...
estdf <- fitdf %>% gather(flow, value, -year) %>%
  group_by(flow) %>%
  arrange(year) %>%
  summarise(pv=plist$pvf(value, plist$dr),
            pv.m1=plist$pvf(value, plist$dr - .01),
            pv.p1=plist$pvf(value, plist$dr + .01),
            duration=plist$duration(value, plist$dr),
...
obj.pay <- function(x, plist=plist){
  payroll <- plist$gauss(x, plist$years)
  pv_payroll <- plist$pvf(payroll, plist$dr)
  obj <- (pv_payroll - plist$pv_payroll)^2
  return(obj)
}

heq.pay <- function(x, ...) {
  # define equality constraints
  yvals.payroll <- plist$gauss(x, plist$years)
  year1_payroll <- yvals.payroll[1]
  pv_payroll <- plist$pvf(yvals.payroll, plist$dr)
  
  cons.eq <- numeric(2)
  cons.eq[1] <- year1_payroll - plist$year1_payroll
  cons.eq[2] <- pv_payroll - plist$pv_payroll
  return(cons.eq)
}

the_plan %>% select(contains("age")) # retire 51.7, actives 42.5

agediff <- the_plan$age_at_retire - the_plan$actives_age

x0 <- c(agediff, 20, 1.5)
xlb <- c(agediff / 2, 0, 0)
xub <- c(2*agediff, 50, 5*plist$year1_payroll)
(x0 >= xlb) & (x0 <= xub)

obj.pay(x0, plist)
heq.pay(x0, plist)

opt <- slsqp(x0, obj.pay, lower=xlb, upper=xub, heq=heq.pay, control=list(xtol_rel = 1e-6), plist=plist)
str(opt)
opt$par %>% round(2)

paydf <- data_frame(year=plist$years, payroll=plist$gauss(opt$par[1:3], year))
paydf

paydf %>% ggplot(aes(year, payroll)) + 
  geom_line() + 
  scale_y_continuous(name="Indexed to year1 total=1", breaks=seq(0, 50, 0.5)) +
  scale_x_continuous(breaks=seq(0, 200, 10)) + 
  ggtitle(paste0("Estimated payroll: ", plist$planname))

paydf %>% ggplot(aes(year, payroll * plist$scale / 1e3)) + 
  geom_line() + 
  scale_y_continuous(name="$ billions", breaks=seq(0, 50, 0.25)) +
  scale_x_continuous(breaks=c(seq(0, 25, 5), seq(30, 200, 10))) + 
  ggtitle(paste0("Estimated payroll: ", plist$planname))



#****************************************************************************************************
#                Alternative approaches - do not work well unless noted ####
#****************************************************************************************************

hin_auglag <- function(x, ...) {
  # define inequality constraints
...
auglag(x0, objfn, lower=xlb, upper=xub, heq=heq, hin=hin, localsolver = c("COBYLA"), localtol = 1e-6, plist=plist)
auglag(x0, objfn, lower=xlb, upper=xub, heq=heq, hin=hin_auglag, localsolver = c("LBFGS"), localtol = 1e-6, plist=plist)
# COBYLA looks bad; LBFGS good but takes a long time; MMA is bad
...
alt <- cobyla(x0, objfn, lower=xlb, upper=xub, hin=hin_auglag, control=list(xtol_rel = 1e-6), plist=plist)
alt$par %>% round(2)

alt <- isres(x0, objfn, lower=xlb, upper=xub, hin=hin, control=list(xtol_rel = 1e-6), plist=plist)

alt <- mma(x0, objfn, lower=xlb, upper=xub, hin=hin_auglag, control=list(xtol_rel = 1e-6), plist=plist)
str(alt)
alt$par %>% round(2) # not good

nloptr(x0, objfn, lb = xlb, ub = xub, 
       eval_g_eq=heq, eval_g_ineq = hin,
